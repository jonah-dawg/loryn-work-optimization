...
- **Pipedrive - active next candidate:** direct tenant inspection confirms a 14-day no-billing trial that starts on Premium with LeadBooster, Smart Docs, and Projects included. Lite is visible at `$24` per seat per month on monthly billing or `$14` per seat per month billed annually, but it has not been selected. A separate no-billing Premium-to-Lite switch gate is required before configuration; Lite's 30-company-custom-field limit and lack of hard required fields still require direct proof.
