...
- **Less Annoying CRM - proposed `Eliminated` result; CP-009 unapproved:** D-112 authorized the bounded no-signup public-demo inspection. Direct inspection confirmed a temporary vendor demo, Contact-scoped Tasks, multiple pipelines, custom fields, global required-field control, and Workspace widgets for due Tasks, pipeline summaries, activity, and calendar items. The pipeline widget exposes aggregate status counts rather than the actionable waiting and missing-next-action records required alongside today's work. M-01 therefore fails directly, and the stopping rule ended the run before any account, trial, configuration, fixtures, broader gates, or scoring. The proposed `Eliminated` result awaits explicit CP-009 sign-off.
...
- [ ] Review and explicitly sign off or reject CP-009 for the P1-W06 public-demo evidence and proposed `Eliminated` result.
