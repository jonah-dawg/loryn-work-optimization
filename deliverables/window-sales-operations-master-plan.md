...
- **Bigin Premier - approved P1-W04 outcome; contract pending:** current official evidence lists 25 custom fields per module, five Team Pipelines, unique fields, Stage Transition Rules, advanced date automation, mobile apps, import/export, paid-edition backups, and developer APIs. Current US pricing is `$15` per user monthly or `$12` per user per month billed annually, and the 15-day trial requires no credit card. D-100 authorizes contract drafting, field mapping, and official research only; the contract, account, and trial remain unapproved.
