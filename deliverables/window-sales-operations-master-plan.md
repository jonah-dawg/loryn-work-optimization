...
- **Bigin Premier - proposed next candidate:** current official evidence lists 25 custom fields per module, five Team Pipelines, unique fields, Stage Transition Rules, advanced date automation, mobile apps, import/export, paid-edition backups, and developer APIs. Current US pricing is `$15` per user monthly or `$12` per user per month billed annually, and the 15-day trial requires no credit card. These capabilities directly target Pipedrive's two mandatory failures, but P1-W04 remains an unapproved proposed outcome and no account or trial is authorized.
