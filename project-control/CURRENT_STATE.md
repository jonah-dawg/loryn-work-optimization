...
- Direct pipeline configuration exposed statuses, multiple pipelines, custom fields, a global required checkbox, and report/badge display, but no visible conditional or unique control in the inspected field dialog.
- M-01 fails directly; the mandatory stopping rule ended the run before account authorization, configuration, fixtures, broader gates, or scoring.
